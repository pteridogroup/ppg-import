...
load_raw_wf <- function(path) {
  readr::read_delim(
    ...
    mutate(
      across(
        c(scientificName, namePublishedIn, synonyms, taxonRemarks),
        ~ unescape_html(.x)
      )
    ) %>%
...
clean_wf <- function(wf_with_syn, wf_syns, rank_by_num, wf_dwc_no_parentage) {
  ...
  autonyms_acc <-
    ...
    separate(
      taxon,
      c("genericName", "specificEpithet", "infraspecificEpithet"),
      sep = " "
    ) %>%
  ...
  higher_taxa_taxon_id_map_acc <- bind_rows(
    ...
    filter(wf_dwc_acc_only_no_higher_tax, taxonRank == "subfamily") %>%
      ...
      left_join(
        unique(
          select(higher_taxa, subfamily, family, order)
        ),
        by = "subfamily"
      ) %>%
    ...
    filter(wf_dwc_acc_only_no_higher_tax, taxonRank == "tribe") %>%
      ...
      left_join(
        unique(
          select(higher_taxa, tribe, subfamily, family, order)
        ),
        by = "tribe"
      ) %>%
  ...
  wf_dwc %>%
    filter(
      !(scientificName == "Hypodematiaceae Ching" &
        taxonomicStatus == "synonym")
    ) %>%
    filter(
      !(scientificName == "Acrostichum L." & taxonomicStatus == "synonym")
    ) %>%
    filter(
      !(scientificName == "Lomariopsis Fée" & taxonomicStatus == "synonym")
    ) %>%
    filter(
      !(scientificName == "Hymenophylloideae Burnett" &
        taxonomicStatus == "synonym")
    ) %>%
    # Arrange by sciname
...
prep_ipni_query <- function(wf_dwc) {
  ...
  wf_dwc <-
    wf_dwc |>
    filter(!str_detect(scientificName, "incertae sedis"))
